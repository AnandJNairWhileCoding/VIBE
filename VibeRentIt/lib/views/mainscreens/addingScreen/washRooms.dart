...
onPressOfContinue(TextEditingController controller) {
  try {
    ...
    if (controller.text == "") {
      showErrorMessage("Oops!", "Please enter the number of wash rooms");
    } else {
    int.parse(controller.text);//to check whether the user has entered an valid input
      ...
      addingScreenController.setScreenState(AddingScreenState.carpetArea);
      ResidenceDetails residenceDetails = Get.find();
      residenceDetails.washRoom  = controller.text;
      controller.text = "";
      controller = null;
    }
  } catch (e) {
...
Widget washRoom(BuildContext context) {
  return Column(
    children: [
      //TEXT INPUT FIELD
      Padding(
        ...
        child: neumorphicTextField("No of washrooms",controller,TextInputType.number,3),
      ),
